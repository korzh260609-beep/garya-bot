...
const pathsToCheck = [
  {
    name: "Относительно index.js (с src)",
  ...
  {
    name: "Абсолютный путь Render (одинарный src)",
    path: '/opt/render/project/src/bot/telegramTransport.js'
  },
  {
    name: "Абсолютный путь Render (двойной src)",
...
// Проверим содержимое папок
console.log("\n📁 Содержимое ключевых папок:");

const dirsToList = [
  { name: "Корень (где index.js)", path: __dirname },
  { name: "src (относительно index.js)", path: join(__dirname, 'src') },
  { name: "Рабочая директория", path: process.cwd() }
];

dirsToList. forEach(dir => {
  console.log(`\n📁 ${dir.name}:`);
  console.log(`   Путь: ${dir. path}`);
  try {
    if (existsSync(dir.path)) {
      const contents = readdirSync(dir. path);
      console.log(`   Содержимое (первые 15):`);
      contents.slice(0, 15).forEach(item => console.log(`      - ${item}`));
      if (contents.length > 15) {
        console.log(`      ... и ещё ${contents.length - 15} файлов/папок`);
      }
    } else {
...
// Проверим папку src/bot если она есть
console.log("\n📁 Содержимое src/bot (если есть):");
const botPath = join(__dirname, 'src', 'bot');
try {
  if (existsSync(botPath)) {
    const botContents = readdirSync(botPath);
    botContents.forEach(item => console.log(`   - ${item}`));
  } else {
    console.log("   ❌ Папка src/bot не существует");
  }
} catch (e) {
  console.log(`   ⚠️ Ошибка: ${e.message}`);
}
