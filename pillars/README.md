...
## Modules
`modules/` starts empty except for its README. Module documentation is created only when the corresponding SG 2.1 module is introduced through the active roadmap. TWM1 is now introduced at architecture/roadmap/workflow level; implementation-specific module docs are created when its code implementation starts.
