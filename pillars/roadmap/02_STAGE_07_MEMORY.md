# ROADMAP — STAGE 7 MEMORY FOUNDATION
...
# STAGE 7.8 — LONG-TERM MEMORY CORE
...
# STAGE 7.9 — LONG-TERM MEMORY RUNTIME MINIMUM
...
- New complex feature work should not continue until 7.8 and 7.9 have reliable runtime minimum.

Purpose:
- make memory useful in real runtime, not only as tables.
...
Gate:
- Project development work should restore Project Memory Core first.
- Repo/GitHub real integration remains later.
- Project memory may prepare evidence contracts now, but must not require real GitHub fetchers.

Purpose:
- allow SG to continue project work across chats without relying on fragile pasted context.
...
7B.10.3 safe-truncate snippets  
7B.10.4 ban verbatim quotes for cross-group  

## 7B.11 GROUPS AS SOURCES — CHAT REGISTRY META

7B.11.1 chat_meta table  
7B.11.2 alias required for group-source  
7B.11.3 title stored as service field, not shown by default  

Gate:
- Local history and idempotent logging are memory core.
- Cross-group recall remains forbidden until later recall/privacy gates exist.

Purpose:
- store dialogue safely without confusing raw chat with confirmed memory.
