...
- live listing then exposed that visible numbering included cancelled and active schedules, while workflow numbered resolution filtered semantic attributes before applying `position`; `position=2` plus active status therefore indexed a one-item filtered set and returned zero. Resolution now selects from the exact visible all-status schedule order first and only then validates any additional explicitly supplied attributes.
