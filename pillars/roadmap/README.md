...
39. `PROJECT_DEVELOPMENT_KNOWLEDGE_4_0_PROGRAM.md` — in-progress PDK4 program; PDK4.1–PDK4.9 CLOSED, PDK4.10–PDK4.12 planned
...
**Status: In progress; PDK4.1–PDK4.9 CLOSED and CI-verified.**
...
PDK4 builds on the completed Project Memory 3.0 foundation. PDK4.1 provides the development contract/taxonomy; PDK4.2 provides bounded historical GitHub scanning with durable PostgreSQL cursor/checkpoints, restart/resume and replay-idempotent processed-source bookkeeping; PDK4.3 provides read-only verified normalization of commits/diffs, PRs, workflow/CI runs and canonical repository files with immutable identities, repository allowlist, secret-redacted bounded content and explicit source/code/CI evidence separation; PDK4.4 provides deterministic significance filtering with bounded AI Router assistance only for ambiguous changes, suppressing generated/formatting/trivial churn while retaining significant architecture/product evidence without changing source trust or evidence state; PDK4.5 converts only event-eligible verified evidence into bounded provenance-backed `DevelopmentEvent` objects and unverified/proposed PM3 candidates, with Router-only semantic enrichment and evidence-gated lifecycle states; PDK4.6 clusters compatible extracted events into deterministic bounded milestone candidates while preserving every atomic event/source, enforcing hard project/domain/component/time separation, attaching explicit `belongs-to-milestone` links, retaining supporting CI/workflow evidence only for audit/correlation and using AI Router solely for ambiguous merge/split assistance; PDK4.7 deterministically rebuilds `ProjectGenesis`, `ProductTimeline`, component histories and development phases from complete PDK4.5/PDK4.6 evidence, preserves superseded historical truth, remains non-authoritative, and records only earliest-known verified evidence when an exact project creation date is not proven; PDK4.8 deterministically links bounded temporal/causal relations, reconciles source/roadmap, code, test/CI, deployment and runtime evidence without collapsing their states, preserves explicit supersession/dependency relations, and emits non-authoritative `DevelopmentKnowledgeGap` records/candidates for stale plans, missing supersession, missing evidence and contradictory evidence chronology instead of guessing; PDK4.9 adds durable incremental GitHub ingestion after a completed PDK4.2 bootstrap, anchors the first incremental poll to the immutable historical commit SHA, accepts bounded poll/webhook/event triggers, treats webhook delivery as trigger-only, independently re-verifies every unseen commit, records processed source/trigger state durably across restart, allows retry only after failed trigger processing, records suppressed/non-event commits without creating PM3 facts, and stores event-eligible output only as unconfirmed PM3 candidates while keeping reconciliation non-authoritative. Canonical documents cannot by themselves prove implementation, CI cannot imply deployment/live state, supporting evidence cannot silently promote milestone state, and deployment/runtime connectors remain unavailable until real approved connectors exist. It reuses PM3 PostgreSQL storage for durable project facts, provenance, trust, confirmation, dedup/conflict, temporal history, retrieval and Context Guard and cannot create a parallel memory or authority system.

PDK4.9 code/tests passed SG 2.1 CI #7129 on commit `963ca2c1dec84fdff6e582e49df7b72cbb3d6500` before canonical documentation synchronization.
