...
port="${PORT:-10000}"
primary_model="${OPENCLAW_PRIMARY_MODEL:-openai/gpt-5.6-terra}"
config_path="$state_dir/openclaw.json"
gateway_public_origin="https://sg-2-2-openclaw.onrender.com"
render_trusted_proxies='["10.192.0.0/13"]'
notion_mcp_server='{"url":"https://mcp.notion.com/mcp","transport":"streamable-http","auth":"oauth","oauth":{"identity":"per-requester"}}'
telegram_groups='{"*":{"requireMention":true,"tools":{"deny":["notion__connect"]}}}'
...
native_project_key="github.com/korzh260609-beep/garya-bot"
SG22_NATIVE_PROJECT_KEY="$native_project_key" node /app/scripts/sg22-migrate-project-memory.mjs
...
config_batch='[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.port","value":'"${port}"'},{"path":"gateway.auth.mode","value":"token"},{"path":"gateway.publicOrigin","value":"'"${gateway_public_origin}"'"},{"path":"gateway.trustedProxies","value":'"${render_trusted_proxies}"'},{"path":"agents.defaults.model.primary","value":"'"${primary_model}"'"},{"path":"agents.defaults.repoRoot","value":"'"${project_repo_root}"'"},{"path":"agents.defaults.compaction.enabled","value":true},{"path":"agents.defaults.compaction.mode","value":"safeguard"},{"path":"agents.defaults.compaction.keepRecentTokens","value":12000},{"path":"agents.defaults.compaction.recentTurnsPreserve","value":4},{"path":"agents.defaults.compaction.identifierPolicy","value":"off"},{"path":"agents.defaults.compaction.qualityGuard","value":{"enabled":true,"maxRetries":1}},{"path":"agents.defaults.compaction.midTurnPrecheck","value":{"enabled":true}},{"path":"agents.defaults.compaction.memoryFlush.enabled","value":true},{"path":"agents.defaults.compaction.maxActiveTranscriptBytes","value":"128kb"},{"path":"agents.defaults.compaction.notifyUser","value":false},{"path":"agents.defaults.contextPruning.mode","value":"cache-ttl"},{"path":"agents.defaults.contextPruning.ttl","value":"5m"},{"path":"agents.defaults.contextPruning.hardClear.enabled","value":true},{"path":"session.dmScope","value":"per-channel-peer"},{"path":"tools.profile","value":"full"},{"path":"tools.alsoAllow","value":'"${workspace_plugin_tools}"'},{"path":"tools.toolsBySender","value":'"${workspace_sender_tools}"'},{"path":"auth.order.openai","value":["openai:api-key"]},{"path":"memory.search.provider","value":"openai"},{"path":"memory.search.remote.apiKey","value":{"source":"env","provider":"default","id":"OPENAI_API_KEY"}},{"path":"mcp.servers.notion","value":'"${notion_mcp_server}"'},{"path":"messages.groupChat.mentionPatterns","value":["(^|[\\s,.:;!?])сг([\\s,.:;!?]|$)","(^|[\\s,.:;!?])sg([\\s,.:;!?]|$)"]},{"path":"tts.provider","value":"openai"},{"path":"tts.auto","value":"off"},{"path":"channels.telegram.dmPolicy","value":"open"},{"path":"channels.telegram.allowFrom","value":["*"]},{"path":"channels.telegram.groupPolicy","value":"allowlist"},{"path":"channels.telegram.groups","value":'"${telegram_groups}"'},{"path":"plugins.load.paths","value":'"${workspace_plugin_paths}"'},{"path":"plugins.entries.sg-workspace-manager.enabled","value":'"${workspace_plugin_enabled}"'},{"path":"plugins.entries.sg-workspace-manager.hooks.allowPromptInjection","value":true},{"path":"plugins.entries.sg-workspace-manager.hooks.allowConversationAccess","value":true}]'

config_batch="${config_batch%]} ,{\"path\":\"messages.visibleReplies\",\"value\":\"automatic\"},{\"path\":\"messages.groupChat.visibleReplies\",\"value\":\"automatic\"}]"
...
if [ "$workspace_plugin_enabled" = "true" ]; then
  for plugin_file in index.ts register.ts personal-memory-tools.ts resource-memory-tools.ts scoped-memory-entries.ts cost-diagnostics.ts render-tools.ts billing-tools.ts phase11-capability-tools.ts openclaw.plugin.json package.json; do
