...
function scopeMatches(request) {
  const { scope } = request;
  return scope.userScope === scope.requestedUserScope
    && scope.projectScope === scope.requestedProjectScope
    && scope.groupScope === scope.requestedGroupScope
    && scope.threadScope === scope.requestedThreadScope;
}

function availabilityCheck(required, available) {
  return required.every((name) => includesWildcard(available, name));
}

function hasValidConfirmation(request) {
  return request.confirmation?.confirmed === true
    && request.confirmation?.requestId === request.traceContext.requestId;
}

function downgradedClass(outcome) {
  if (outcome === 'downgrade-to-analysis') return 'analysis-only';
  if (outcome === 'downgrade-to-prepare') return 'prepare-only';
  return null;
}
  ...
  return Object.freeze({
    ...
    evaluate(actionRequest, { policyContext = null } = {}) {
      ...
      if (outcome === 'allow' && actionRequest.idempotencyKey) {
        const reserved = idempotencyStore.reserve(actionRequest.idempotencyKey, {
          traceId: actionRequest.traceContext.traceId,
          requestId: actionRequest.traceContext.requestId,
      ...
      const effectiveActionClass = downgradedClass(outcome) ?? actionRequest.actionClass;
