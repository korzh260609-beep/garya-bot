...
export function buildRepoStateSemanticMapReport({ command, result, collectedAt }) {
  ...
  const immediateChecks = asArray(nextActionPlan?.immediateChecks);
  const suggestedNextSteps = asArray(nextActionPlan?.suggestedNextSteps);
  const blockedActions = asArray(nextActionPlan?.blockedActions);
  const architectureFindings = asArray(architectureHealth?.findings);
  const architectureFocus = asArray(architectureHealth?.recommendedFocus);
  ...
  const immediateCheckRows = immediateChecks.map((item) => [
    item.id || "-",
    item.title || "-",
    item.target || "-",
    item.reason || "-",
  ]);
  const suggestedNextStepRows = suggestedNextSteps.map((item) => [
    item.id || "-",
    item.priority || "-",
    item.type || "-",
    item.title || "-",
  ]);
  const architectureFindingRows = architectureFindings.map((item) => [
    item.id || "-",
    item.severity || "-",
    item.area || "-",
    item.title || "-",
  ]);
  const architectureFocusRows = architectureFocus.map((item) => [
    item.findingId || "-",
    item.priority || "-",
    item.area || "-",
    item.title || "-",
...
export function buildRepoStateAgentTestReport({ command, result, collectedAt, resultStatus }) {
  ...
  const nextActionPlan = result?.nextActionPlan || null;
  const architectureHealth = result?.architectureHealth || null;
  ...
  return `# TEST_REPORT\n\nSG full repo state agent result after workspace command execution.\n\n---\n\nTask ID: \`${command.taskId || "manual"}\`\nDeploy ID: \`${command.deployId || "-"}\`\nCommit: \`-\`\nTested at: \`${collectedAt}\`\nTested by: \`SG AgentWorkspaceCommandRunner\`\n\n---\n\n## Test command\n\n\`\`\`text\nRUN_REPO_STATE_AGENT\n\`\`\`\n\n## Result\n\n- \`${reportResultStatus}\`\n\n## Technical map\n\n\`\`\`text\nok: ${result?.ok === true ? "yes" : "no"}\npersisted: ${result?.persisted === true ? "yes" : "no"}\nrepo: ${result?.repoFullName || "-"}\nbranch: ${result?.branch || "-"}\nfiles: ${result?.filesCount ?? "-"}\nmodules: ${result?.modulesCount ?? "-"}\ndependencies: ${result?.dependenciesCount ?? "-"}\nprojectMap: ${projectMap ? "yes" : "no"}\nprojectMapSchemaVersion: ${projectMap?.schemaVersion || "-"}\nprojectMapModules: ${Array.isArray(projectMap?.modules) ? projectMap.modules.length : "-"}\nprojectMapLinks: ${Array.isArray(projectMap?.moduleLinks) ? projectMap.moduleLinks.length : "-"}\nsemanticMap: ${semanticMap ? "yes" : "no"}\nsemanticMapSchemaVersion: ${semanticMap?.schemaVersion || "-"}\nsemanticMapModules: ${Array.isArray(semanticMap?.modulePurposes) ? semanticMap.modulePurposes.length : "-"}\nsemanticMapTaskRoutingHints: ${Array.isArray(semanticMap?.taskRoutingHints) ? semanticMap.taskRoutingHints.length : "-"}\nsemanticMapTaskSafetyGates: ${Array.isArray(semanticMap?.taskSafetyGates) ? semanticMap.taskSafetyGates.length : "-"}\nsemanticMapRecommendedReadOrder: ${Array.isArray(semanticMap?.recommendedReadOrder) ? semanticMap.recommendedReadOrder.length : "-"}\nsemanticMapBoundaryRules: ${Array.isArray(semanticMap?.boundaryRules) ? semanticMap.boundaryRules.length : "-"}\nsemanticMapRiskHints: ${Array.isArray(semanticMap?.riskHints) ? semanticMap.riskHints.length : "-"}\nnextActionPlan: ${nextActionPlan?.schemaVersion ? "yes" : "no"}\nnextActionPlanSchemaVersion: ${nextActionPlan?.schemaVersion || "-"}\nnextActionPlanImmediateChecks: ${Array.isArray(nextActionPlan?.immediateChecks) ? nextActionPlan.immediateChecks.length : "-"}\nnextActionPlanSuggestedNextSteps: ${Array.isArray(nextActionPlan?.suggestedNextSteps) ? nextActionPlan.suggestedNextSteps.length : "-"}\nnextActionPlanBlockedActions: ${Array.isArray(nextActionPlan?.blockedActions) ? nextActionPlan.blockedActions.length : "-"}\nnextActionPlanRecommendedReadOrder: ${Array.isArray(nextActionPlan?.recommendedReadOrder) ? nextActionPlan.recommendedReadOrder.length : "-"}\narchitectureHealth: ${architectureHealth?.schemaVersion ? "yes" : "no"}\narchitectureHealthSchemaVersion: ${architectureHealth?.schemaVersion || "-"}\narchitectureHealthScore: ${architectureHealth?.score ?? "-"}\narchitectureHealthStatus: ${architectureHealth?.status || "-"}\narchitectureHealthFindings: ${Array.isArray(architectureHealth?.findings) ? architectureHealth.findings.length : "-"}\narchitectureHealthRecommendedFocus: ${Array.isArray(architectureHealth?.recommendedFocus) ? architectureHealth.recommendedFocus.length : "-"}\nscanRunId: ${result?.persistence?.scanRunId || "-"}\nerror: ${result?.error || result?.persistence?.error || "-"}\n\`\`\`\n\n## Semantic AI map\n\n\`\`\`text\naiEnabled: ${ai?.enabled === true ? "yes" : "no"}\naiSkipped: ${ai?.skipped === true ? "yes" : "no"}\naiReused: ${ai?.reused === true ? "yes" : "no"}\naiDryRun: ${ai?.aiDryRun === true ? "yes" : "no"}\ntokensSpent: ${ai?.tokensSpent === true ? "yes" : "no"}\naiSource: ${ai?.aiSource || "unknown"}\naiReason: ${ai?.reason || meta?.reason || "-"}\nshouldAnalyze: ${meta?.shouldAnalyze === true ? "yes" : "no"}\naiForceAnalysis: ${meta?.forceAiAnalysis === true || ai?.forceAiAnalysis === true ? "yes" : "no"}\nallowRealAi: ${meta?.allowRealAi === true || ai?.allowRealAi === true ? "yes" : "no"}\nrealAiBlocked: ${meta?.realAiBlocked === true ? "yes" : "no"}\nblockReason: ${meta?.realAiBlocked === true ? "missing_allow_real_ai" : "-"}\naiModel: ${aiModel}\naiUsedFallback: ${aiUsedFallback ? "yes" : "no"}\naiInputTokens: ${formatNullableNumber(aiInputTokens)}\naiOutputTokens: ${formatNullableNumber(aiOutputTokens)}\naiTotalTokens: ${formatNullableNumber(aiTotalTokens)}\naiPricingConfigured: ${aiPricingConfigured ? "yes" : "no"}\naiEstimatedCostUsd: ${formatUsd(aiEstimatedUsd)}\noriginalShouldAnalyze: ${meta?.originalShouldAnalyze === true || ai?.originalShouldAnalyze === true ? "yes" : "no"}\npromptChars: ${promptChars}\nsignatureLength: ${meta?.projectMapSignature ? String(meta.projectMapSignature.length) : "-"}\nhasAnalysis: ${ai?.analysis ? "yes" : "no"}\n\`\`\`\n\n## Raw compact\n\n\`\`\`json\n${safeJson({
    ...
    nextActionPlan: nextActionPlan ? {
      schemaVersion: nextActionPlan.schemaVersion || null,
      generatedBy: nextActionPlan.generatedBy || null,
      tokensSpent: nextActionPlan.tokensSpent === true,
      immediateChecks: Array.isArray(nextActionPlan.immediateChecks) ? nextActionPlan.immediateChecks.length : 0,
      suggestedNextSteps: Array.isArray(nextActionPlan.suggestedNextSteps) ? nextActionPlan.suggestedNextSteps.length : 0,
      blockedActions: Array.isArray(nextActionPlan.blockedActions) ? nextActionPlan.blockedActions.length : 0,
      recommendedReadOrder: Array.isArray(nextActionPlan.recommendedReadOrder) ? nextActionPlan.recommendedReadOrder.length : 0,
    } : null,
    architectureHealth: architectureHealth ? {
      schemaVersion: architectureHealth.schemaVersion || null,
      generatedBy: architectureHealth.generatedBy || null,
      tokensSpent: architectureHealth.tokensSpent === true,
      score: architectureHealth.score ?? null,
      status: architectureHealth.status || null,
      findings: Array.isArray(architectureHealth.findings) ? architectureHealth.findings.length : 0,
      recommendedFocus: Array.isArray(architectureHealth.recommendedFocus) ? architectureHealth.recommendedFocus.length : 0,
      counters: architectureHealth.counters || null,
    } : null,
    aiUsage: {
