...
export function createProductionAI({ env = process.env, fetchImpl = globalThis.fetch, telemetry = createInMemoryAITelemetry() } = {}) {
  ...
  const aiRouter = createAIRouter({
    ...
    maxRetries: positiveInteger(env.AI_MAX_RETRIES, 1),
    retryDelayMs: positiveInteger(env.AI_RETRY_DELAY_MS, 100)
  });
  const meaningInterpreter = createProductionMeaningInterpreter({ aiRouter });

  return Object.freeze({ registry, telemetry, aiRouter, meaningInterpreter });
}
