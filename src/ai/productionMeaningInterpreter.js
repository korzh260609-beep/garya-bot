...
export function createProductionMeaningInterpreter({ aiRouter, fallbackOnFailure = false }) {
  ...
  return Object.freeze({
    ...
    async interpret(canonicalInput) {
      ...
      try {
        const result = await aiRouter.route({ task: 'semantic-interpretation', specialty: 'semantic-interpretation', reason: 'Interpret canonical user meaning for Semantic Kernel', traceContext: canonicalInput.traceContext, identityContext: canonicalInput.identityContext, role: canonicalInput.identityContext?.roles?.[0] ?? 'guest', messages: [{ role: 'system', content: `${boundary.system}\n\nRecurring automation identifiers are internal. Never ask the user to read, copy or choose a scheduleId. If the user explicitly refers to a numbered automation from a previously displayed list (for example first, second or number 2), put that one-based number in payload.selector.position. Use position only when the user expressed it; never invent it. The scoped resolver must still fail closed when no explicit position or other selector uniquely identifies a target.\n\nWhen the user asks to add current activity from every Telegram group/workspace where SG and the user currently have authorized access to an existing automation, use automation-update with semanticOperation {type:\"add-workspace-activity\",data:{workspaceSelection:\"authorized-current\"}}. The runtime resolves the current authorized set at execution time and independently reauthorizes every workspace; workspace IDs are neither required from nor shown to the user. This operation preserves the existing notification text and appends a fresh deterministic activity report. Do not refuse merely because the user did not provide workspace IDs.` }, { role: 'user', content: boundary.user }], responseFormat: { name: 'semantic_interpretation', jsonSchema: SEMANTIC_SCHEMA, strict: false }, metadata: { locale: canonicalInput.locale, languageContext: canonicalInput.metadata?.languageContext ?? null, roles: canonicalInput.identityContext?.roles ?? [], context: userPayload } });
