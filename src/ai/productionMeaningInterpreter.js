...
const NULLABLE_SEMANTIC_OBJECT_SCHEMA = Object.freeze({
  anyOf: [
    { type: 'null' },
...
const SEMANTIC_SCHEMA = Object.freeze({
  ...
  properties: {
    ...
    provenance: SEMANTIC_OBJECT_SCHEMA,
...
- Cancellation of a non-recurring task uses task-cancel/state-change. If the user refers to an item in the previously displayed active task list, put the expressed one-based number in payload.selector.position. If the user identifies it by its human description, put that meaning in payload.selector.description. Preserve taskId only when the user actually supplied an internal task id. Never convert a displayed position into a taskId and never guess a target; the deterministic scoped task resolver owns the final match.
...
- A user may identify an existing automation in ordinary language. Put a short description of the EXISTING target in payload.selector.description, excluding the requested new behavior. Preserve explicit current localTime, recurrence or full existing notificationMessage as additional selectors when supplied. Never require an automationId, taskId, scheduleId or list position from the user.
- Description resolution is bounded to the current canonical scope and operational lifecycle states. One confident match executes; zero matches asks for a more precise description; several close matches ask the user to distinguish them by human-readable message/time/schedule, never by an internal ID.
