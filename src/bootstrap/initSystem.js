...
/**
 * INIT SYSTEM (Bootstrapping)
 * Всё, что выполняется 1 раз при старте сервера.
 * Важно: не переписываем логику — переносим 1:1 из index.js.
 */
export async function initSystem({ pool, bot, monarchChatId }) {
  await ensureProjectMemoryTable(pool);
  console.log("🧠 Project Memory table OK.");

  await ensureFileIntakeLogsTable(pool);
  console.log("🧾 File-Intake logs table OK.");

  // access_requests (auto-create)
  ...
  // monarchChatId пока не используется тут напрямую, но оставляем параметром под расширение init
  void monarchChatId;
}

async function ensureProjectMemoryTable(pool) {
  await pool.query(`
...
/**
 * FILE-INTAKE LOGS
 * Таблица и индекс — 1:1 из index.js.
 */
async function ensureFileIntakeLogsTable(pool) {
  await pool.query(`
