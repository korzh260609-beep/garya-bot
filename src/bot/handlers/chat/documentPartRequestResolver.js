// src/bot/handlers/chat/documentPartRequestResolver.js
// ============================================================================
// STAGE 12A.2 — DOCUMENT PART REQUEST RESOLVER
// Purpose:
...
export async function resolveDocumentPartRequest({
  callAI,
  userText,
  estimateContext,
}) {
  ...
  const messages = [
    {
      ...
      content:
        ...
        "The assistant already knows the active document context and already has a split/estimate for that document.\n" +
        "Your task is ONLY to decide whether the user's new message asks to SHOW THE ACTUAL TEXT of ONE SPECIFIC PART of that document directly in chat.\n" +
        "Return ONLY valid JSON.\n\n" +
        "Schema:\n" +
        "{\n" +
        '  "isDocumentPartRequest": boolean,\n' +
        ...
        "}\n\n" +
        "Rules:\n" +
        "- isDocumentPartRequest=true only if the user wants the ACTUAL CONTENT/TEXT of a specific numbered part shown in chat.\n" +
        "- requestedPartNumber must be the exact requested part number if clear; otherwise 0.\n" +
        "- Ordinals like 'first', 'second', 'sixteenth' must be converted to numbers.\n" +
        "- Russian/Ukrainian colloquial ordinal forms also count.\n" +
  ...
  try {
    ...
    return {
      ok: true,
      isDocumentPartRequest: Boolean(parsed?.isDocumentPartRequest),
