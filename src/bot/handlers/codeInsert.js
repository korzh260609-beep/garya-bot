...
export async function handleCodeInsert(ctx) {
  ...
  if (codeOutputMode === CODE_OUTPUT_MODES.DRY_RUN) {
    ...
    if (!path || !anchor || !mode) {
      await bot.sendMessage(
        chatId,
        refuseText("BAD_ARGS", "Формат: /code_insert path | anchor | mode | requirement (mode=before|after|replace)")
      );
      return;
    }
  ...
  // ==========================================================================
  if (codeOutputMode === CODE_OUTPUT_MODES.DISABLED) {
    try {
  ...
  // ==========================================================================
  if (codeOutputMode !== CODE_OUTPUT_MODES.ENABLED) {
  ...
  if (!isMonarch) {
    try {
      console.info("🧾 CODE_REFUSE", { ...aiMetaBase, refuseReason: "NOT_MONARCH" });
    } catch (_) {}
  ...
  if (!isPrivateLike) {
    try {
      console.info("🧾 CODE_REFUSE", { ...aiMetaBase, refuseReason: "NOT_PRIVATE_CHAT" });
    } catch (_) {}
    await bot.sendMessage(chatId, refuseText("PRIVATE_ONLY", "Используй команду только в личном чате с SG."));
    return;
  }
  ...
  if (String(path).length > 300) {
    await bot.sendMessage(chatId, refuseText("PATH_TOO_LONG", "Сократи path (≤ 300 символов)."));
    try {
      console.info("🧾 CODE_REFUSE", { ...aiMetaBase, refuseReason: "PATH_TOO_LONG" });
    } catch (_) {}
    return;
  }
  if (String(path).includes("..") || String(path).startsWith("/") || String(path).startsWith("\\")) {
    await bot.sendMessage(chatId, refuseText("BAD_PATH", "Запрещены .. и абсолютные пути. Дай относительный путь из репо."));
    try {
      console.info("🧾 CODE_REFUSE", { ...aiMetaBase, refuseReason: "BAD_PATH" });
    } catch (_) {}
    return;
  }
  if (denySensitivePath(path)) {
  ...
  if (String(anchor).length > 200) {
    await bot.sendMessage(chatId, refuseText("ANCHOR_TOO_LONG", "Сократи anchor (≤ 200 символов)."));
    try {
      console.info("🧾 CODE_REFUSE", { ...aiMetaBase, refuseReason: "ANCHOR_TOO_LONG" });
    } catch (_) {}
    return;
  }
  if (isDangerousAnchorOrContent(anchor)) {
  ...
  if (String(requirement || "").length > 1200) {
    await bot.sendMessage(chatId, refuseText("REQ_TOO_LONG", "Сократи requirement (≤ 1200 символов)."));
    try {
      console.info("🧾 CODE_REFUSE", { ...aiMetaBase, refuseReason: "REQ_TOO_LONG" });
    } catch (_) {}
    return;
  }
  if (isDangerousAnchorOrContent(requirement)) {
    ...
    try {
      console.info("🧾 CODE_REFUSE", { ...aiMetaBase, refuseReason: "DANGEROUS_REQUIREMENT" });
    } catch (_) {}
    return;
  }
  ...
  if (typeof callAI !== "function") {
    ...
    try {
      console.info("🧾 CODE_REFUSE", { ...aiMetaBase, refuseReason: "INTERNAL_ERROR_CALLAI" });
    } catch (_) {}
    return;
  }
