...
const OPERATIONS = Object.freeze(['summarize-range', 'search', 'first-occurrence', 'last-occurrence', 'timeline']);
const PLAN_SCHEMA = Object.freeze({ type: 'object', additionalProperties: false, required: ['operation'], properties: { operation: { type: 'string', enum: OPERATIONS } } });
...
export async function retrieveLongTermConversationHistory({ store, aiRouter, request, query, temporalRange = null, conversationId = null, topicId = null, pageSize = 160, chunkCharacters = 12000, mergeFanout = 5, evidenceLimit = 8, operationHint = null } = {}) {
  ...
  if (operationHint != null && !OPERATIONS.includes(operationHint)) throw new TypeError('Unsupported Conversation History operationHint');
  ...
  const operation = operationHint ?? await plan({ aiRouter, request, query: semanticQuery });
