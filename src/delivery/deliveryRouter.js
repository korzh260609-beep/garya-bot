...
function transport(value) {
  const normalized = required(value, 'target.transport').toLowerCase();
  if (!TRANSPORTS.has(normalized)) throw new TypeError(`unsupported delivery transport: ${normalized}`);
  return normalized;
}
function normalizeTarget(value, name = 'target') {
  ...
  return Object.freeze({
    ...
    resourceId: optional(value.resourceId),
    connectionId: optional(value.connectionId),
    address: optional(value.address),
    threadId: optional(value.threadId),
    replyToMessageId: optional(value.replyToMessageId),
    metadata: Object.freeze(clone(value.metadata ?? {}))
  });
}
...
export function createInMemoryDeliveryStore() {
  ...
  return Object.freeze({
    async getByIdempotencyKey(key) { const id = byKey.get(key); return id ? clone(byId.get(id)) : null; },
    async put(record) { byId.set(record.deliveryId, clone(record)); byKey.set(record.idempotencyKey, record.deliveryId); return clone(record); },
    async get(deliveryId) { return clone(byId.get(deliveryId) ?? null); }
  });
}
...
export function createDeliveryTransportRegistry({ transports = [] } = {}) {
  ...
  for (const item of transports) {
    const name = transport(item?.name);
    if (map.has(name)) throw new TypeError(`duplicate delivery transport: ${name}`);
    if (typeof item.deliver !== 'function') throw new TypeError(`${name}.deliver is required`);
    map.set(name, Object.freeze({ name, deliver: item.deliver }));
  }
  return Object.freeze({
    get(name) { return map.get(transport(name)) ?? null; },
    register(item) { const name = transport(item?.name); if (map.has(name)) throw new TypeError(`duplicate delivery transport: ${name}`); if (typeof item.deliver !== 'function') throw new TypeError(`${name}.deliver is required`); map.set(name, Object.freeze({ name, deliver: item.deliver })); return this; },
    names: () => Object.freeze([...map.keys()].sort())
  });
}
...
export function createDeliveryRouter({
  ...
  timeoutMs = positiveInt(timeoutMs, 'timeoutMs', 10_000);
  maxAttempts = positiveInt(maxAttempts, 'maxAttempts', 3);

  async function resolveSettings(request) {
    if (!userSettingsService?.resolve) return null;
  ...
  function pickTargets(request, settings) {
    ...
    const preferred = settings?.settings?.delivery?.preferredTransport;
    const targets = [];
  ...
  async function route(input) {
    ...
    if (duplicate) return publicResult({ ...duplicate, duplicate: true });
    const settings = await resolveSettings(request);
    if (request.kind === 'notification') {
      if (settings?.settings?.notifications?.enabled === false) {
        const record = { ...request, status: 'suppressed', attempts: 0, failureCode: 'notifications-disabled', retryable: false };
        await store.put(record); await recordEvent(request, 'suppressed', { reason: record.failureCode }); return publicResult(record);
      }
      if (isQuietHour(settings?.settings, clock())) {
        const record = { ...request, status: 'deferred', attempts: 0, failureCode: 'quiet-hours', retryable: true };
        await store.put(record); await recordEvent(request, 'deferred', { reason: record.failureCode }); return publicResult(record);
      }
    }
    const targets = pickTargets(request, settings);
    if (targets.length === 0) {
      const record = { ...request, status: 'failed', attempts: 0, failureCode: 'delivery-target-unavailable', retryable: false };
      await store.put(record); await recordEvent(request, 'failed', { reason: record.failureCode }); return publicResult(record);
    }
    let totalAttempts = 0; let lastError = null;
    for (const target of targets) {
      const auth = await authorizeTarget(request, target);
      if (!auth.allowed) { lastError = Object.assign(new Error(auth.reason), { code: auth.reason, retryable: false }); await recordEvent(request, 'denied', { transport: target.transport, reason: auth.reason }); continue; }
      const provider = transportRegistry.get(target.transport);
      if (!provider) { lastError = Object.assign(new Error('delivery transport unavailable'), { code: 'delivery-transport-unavailable', retryable: false }); continue; }
      for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        totalAttempts += 1;
        try {
          const result = await withTimeout(() => provider.deliver(Object.freeze({ request, target, attempt, locale: request.locale ?? settings?.settings?.locale ?? null })), timeoutMs);
          const record = { ...request, target, transport: target.transport, status: 'delivered', attempts: totalAttempts, failureCode: null, retryable: false, deliveredAt: clock().toISOString(), providerResult: clone(result ?? null) };
          await store.put(record); await recordEvent(request, 'delivered', { transport: target.transport, attempts: totalAttempts }); return publicResult(record);
        } catch (error) {
          lastError = error; const retryable = isRetryable(error);
          await recordEvent(request, 'attempt-failed', { transport: target.transport, attempt, code: error?.code ?? 'delivery-failed', retryable });
          if (!retryable) break;
        }
      }
    }
