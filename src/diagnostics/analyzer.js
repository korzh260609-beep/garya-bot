...
function failureCode(evidence) {
  return evidence.errorCode
    ?? evidence.payload?.data?.code
    ?? evidence.payload?.data?.error?.code
...
export function findFirstDivergence(trace) {
  ...
  for (let index = 0; index < stages.length; index += 1) {
    ...
    return Object.freeze({
      ...
      evidenceIds: Object.freeze(current.evidence.map((item) => item.evidenceId)),
      previousCompleted
    });
  }
  return null;
...
function gateDeniedIsExpected(stage, evidence) {
  ...
  return evidence.some((item) => {
    const outcome = String(item.payload?.outcome ?? item.payload?.data?.gateOutcome ?? '').toLowerCase();
    return ['deny', 'denied', 'confirmation-required', 'downgrade'].includes(outcome);
  });
}
...
export function analyzeRootCause({ trace, firstDivergence, deploymentFindings = [] } = {}) {
  ...
  if (deploymentConfirmed) {
    return Object.freeze({
      ...
      component: deploymentConfirmed.component,
      confidence: 'CONFIRMED',
      summary: deploymentConfirmed.summary,
      evidenceIds: deploymentConfirmed.evidenceIds ?? [],
      data: deploymentConfirmed.data ?? {}
    });
  }
  if (!firstDivergence) return null;
  const stage = trace.stages[firstDivergence.index];
  if (gateDeniedIsExpected(stage.stage, stage.evidence)) {
    return Object.freeze({
      errorClass: 'ACTION_GATE', component: 'action-gate', confidence: 'CONFIRMED',
      summary: 'Action Gate stopped the request according to an explicit gate outcome.',
      evidenceIds: stage.evidence.map((item) => item.evidenceId), data: { expectedControlOutcome: true }
    });
  }
  ...
  if (primary) {
    ...
    return Object.freeze({
      errorClass,
      component: primary.component ?? stage.stage,
      confidence: failureCode(primary) ? 'CONFIRMED' : 'HIGH',
      summary: `${stage.stage} ${stage.status}${failureCode(primary) ? `: ${failureCode(primary)}` : ''}`,
      evidenceIds: stage.evidence.map((item) => item.evidenceId),
      data: { stage: stage.stage, status: stage.status, errorCode: failureCode(primary) }
    });
  }
  const childFailure = trace.evidence.find((item) => failureLike(item));
  if (childFailure) {
    return Object.freeze({
      errorClass: classifyFailure(stage.stage, childFailure),
      component: childFailure.component ?? stage.stage,
      confidence: 'MEDIUM',
      summary: `Expected stage ${stage.stage} is missing; a correlated failure exists in the trace.`,
      evidenceIds: [childFailure.evidenceId],
      data: { missingStage: stage.stage, correlatedErrorCode: failureCode(childFailure) }
    });
  }
  return Object.freeze({
    errorClass: CLASS_BY_STAGE[stage.stage] ?? 'UNKNOWN',
    component: stage.stage,
    confidence: 'LOW',
    summary: `Expected stage ${stage.stage} has no matching completion evidence.`,
    evidenceIds: [],
    data: { missingStage: stage.stage }
  });
}

export function buildFindings({ trace, firstDivergence, rootCause, deploymentFindings = [] } = {}) {
  const findings = [...deploymentFindings];
  if (firstDivergence) findings.push(createDiagnosticFinding({
    kind: 'first-divergence',
    errorClass: rootCause?.errorClass ?? CLASS_BY_STAGE[firstDivergence.stage] ?? 'UNKNOWN',
    component: firstDivergence.stage,
    confidence: firstDivergence.evidenceIds.length ? 'HIGH' : 'MEDIUM',
    summary: `First divergence at ${firstDivergence.stage}: ${firstDivergence.status}.`,
    evidenceIds: firstDivergence.evidenceIds,
    data: { expectedPathId: trace.expectedPathId, index: firstDivergence.index }
  }));
  if (rootCause) findings.push(createDiagnosticFinding({
    kind: 'root-cause', errorClass: rootCause.errorClass, component: rootCause.component,
    confidence: rootCause.confidence, summary: rootCause.summary,
    evidenceIds: rootCause.evidenceIds, data: rootCause.data
  }));
...
export function downstreamEffects(trace, firstDivergence) {
  ...
  return Object.freeze(trace.stages.slice(firstDivergence.index + 1)
    .filter((item) => item.status !== 'completed')
    .map((item) => Object.freeze({ stage: item.stage, status: item.status })));
}
