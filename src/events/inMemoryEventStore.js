...
export function createInMemoryEventStore() {
  ...
  return Object.freeze({
    async appendEvent(event) {
      if (!events.has(event.eventId)) events.set(event.eventId, clone(event));
      return clone(events.get(event.eventId));
    },
    async getEvent(eventId) { return events.has(eventId) ? clone(events.get(eventId)) : null; },
    async registerSubscription(subscription) {
      subscriptions.set(subscription.subscriberId, clone(subscription));
    ...
    async claimPending({ limit = 20, now = new Date().toISOString() } = {}) {
      const ready = [...deliveries.values()]
        ...
        .sort((a, b) => `${a.createdAt}:${a.deliveryId}`.localeCompare(`${b.createdAt}:${b.deliveryId}`))
        .slice(0, limit);
      for (const item of ready) {
        const key = `${item.eventId}:${item.subscriberId}`;
        deliveries.set(key, { ...item, status: 'processing', updatedAt: now });
      }
    ...
    async markDelivered({ eventId, subscriberId, attempts, deliveredAt }) {
      const key = `${eventId}:${subscriberId}`;
      const current = deliveries.get(key);
      if (!current) return null;
      const next = { ...current, status: 'delivered', attempts, deliveredAt, failureCode: null, nextAttemptAt: null, updatedAt: deliveredAt };
    ...
    async markRetry({ eventId, subscriberId, attempts, failureCode, nextAttemptAt, updatedAt }) {
      ...
      const current = deliveries.get(key);
      if (!current) return null;
      const next = { ...current, status: 'pending', attempts, failureCode, nextAttemptAt, updatedAt };
      deliveries.set(key, next);
      return clone(next);
    },
    async markDeadLetter({ eventId, subscriberId, attempts, failureCode, updatedAt }) {
      ...
      const current = deliveries.get(key);
      if (!current) return null;
      const next = { ...current, status: 'dead-letter', attempts, failureCode, nextAttemptAt: null, updatedAt };
      deliveries.set(key, next);
      return clone(next);
    },
    async getDelivery({ eventId, subscriberId }) {
      const item = deliveries.get(`${eventId}:${subscriberId}`);
    ...
    async requeueDeadLetter({ eventId, subscriberId, updatedAt }) {
      const key = `${eventId}:${subscriberId}`;
      const current = deliveries.get(key);
      if (!current || current.status !== 'dead-letter') return null;
      const next = { ...current, status: 'pending', failureCode: null, nextAttemptAt: updatedAt, updatedAt };
      deliveries.set(key, next);
      return clone(next);
    }
  });
