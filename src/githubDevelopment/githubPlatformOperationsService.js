...
const READ_ACTIONS=new Set(['github.repository.inspect','github.file.read','github.branch.inspect','github.branch.compare','github.pr.inspect','github.review.inspect','github.issue.inspect','github.ci.verify','github.workflow.inspect','github.discovery.public','github.discovery.private']);
...
export function createGitHubPlatformOperationsService({repositoryReadService,discoveryService,atomicCommitService,collaborationService,ciService,developmentBridge,taskStore,securityControlPlane,auditSink,clock=()=>new Date()}={}){
  if(!repositoryReadService?.readSnapshot||!discoveryService?.search||!atomicCommitService||!collaborationService||!ciService||!developmentBridge?.execute||!taskStore?.get||!securityControlPlane?.authorize||!auditSink?.record)throw new TypeError('GDE6 existing GH3 services are required');
  async function continuity(input){const taskId=req(input.context?.taskId,'context.taskId');const task=await taskStore.get(taskId);if(!task)fail('gde6-context-not-found','durable development context was not found');if(task.globalUserId!==input.actor?.globalUserId||task.projectId!==input.projectScope)fail('gde6-context-scope-denied','durable development context scope mismatch');return task}
  async function execute(input={}){
    ...
    if(action==='github.development.execute'||action==='github.development.continue')result=await developmentBridge.execute({...input,canonicalModel:input.canonicalModel??{resolutionStatus:'resolved',action:{name:'github.development.execute'},parameters:{instruction:task?.intent}},resolvedTarget:input.resolvedTarget??{repository,branch,baselineHead:oldHead,connectionId:input.connectionId},capabilityAssessment:input.capabilityAssessment});
