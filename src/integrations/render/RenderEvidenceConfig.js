...
function envBool(env, name, fallback = false) {
  const raw = normalizeString(env?.[name] || "").toLowerCase();
...
export function getRenderEvidenceConfig(env = process.env) {
  const enabled = envBool(env, "RENDER_EVIDENCE_ENABLED", false);
  ...
  return {
    ...
    serviceId,
    ownerId,
    timeoutMs: envInt(env, "RENDER_EVIDENCE_TIMEOUT_MS", 20_000, 1_000, 120_000),
    defaultDeployLimit: envInt(env, "RENDER_EVIDENCE_DEPLOY_LIMIT", 5, 1, 20),
    defaultLogLimit: envInt(env, "RENDER_EVIDENCE_LOG_LIMIT", 100, 1, 500),
    defaultLogWindowMinutes: envInt(env, "RENDER_EVIDENCE_LOG_WINDOW_MINUTES", 60, 1, 1440),
