...
export function createLanguageAwareConversationResponder({
  ...
  return async function conversationResponder({ text, request }) {
    ...
    if (projectMemoryIntegration && !identityResponseContract.active) {
      try {
        ...
        if (error?.code === 'project-memory-ai-scope-mismatch'
          ...
          || error?.code === 'project-memory-retrieval-unauthorized') throw error;
        projectMemoryContext = null;
      }
    }

    const modelContext = projectMemoryIntegration
      ? projectMemoryIntegration.prepareModelContext({ boundedResponseContext, projectMemoryContext })
      : { boundedResponseContext, projectMemoryContext: null };
    ...
    if (!aiRouter?.route) {
      ...
      if (projectMemoryContext) return projectMemoryIntegration.deterministicAnswer({ context: projectMemoryContext, responseLanguage });
