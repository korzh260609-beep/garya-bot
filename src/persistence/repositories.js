...
export function createPostgresRepositories(database) {
  const users = Object.freeze({
    async upsert({ globalUserId, profile = {} }, db = database) {
      const result = await db.query(`INSERT INTO users(global_user_id, profile) VALUES ($1,$2::jsonb)
        ON CONFLICT(global_user_id) DO UPDATE SET profile = users.profile || EXCLUDED.profile, updated_at = now() RETURNING *`, [required(globalUserId, 'globalUserId'), JSON.stringify(profile)]);
  ...
  const identities = Object.freeze({
    async link({ platform, platformUserId, globalUserId, metadata = {} }, db = database) {
      ...
      const result = await db.query(`INSERT INTO identity_links(platform, platform_user_id, global_user_id, metadata)
        ...
        WHERE identity_links.global_user_id = EXCLUDED.global_user_id RETURNING *`, [required(platform, 'platform'), required(platformUserId, 'platformUserId'), required(globalUserId, 'globalUserId'), JSON.stringify(metadata)]);
    ...
    async upsert({ conversationId, scope, metadata = {} }, db = database) {
      ...
      const result = await db.query(`INSERT INTO conversations(conversation_id, global_user_id, project_scope, group_scope, thread_scope, metadata)
        ...
        WHERE conversations.global_user_id=EXCLUDED.global_user_id AND conversations.project_scope=EXCLUDED.project_scope
          AND conversations.group_scope IS NOT DISTINCT FROM EXCLUDED.group_scope AND conversations.thread_scope IS NOT DISTINCT FROM EXCLUDED.thread_scope RETURNING *`, [conversationId, globalUserId, projectScope, groupScope, threadScope, JSON.stringify(metadata)]);
    ...
    async appendMessage({ messageId, conversationId, scope, direction, content, provenance = {} }, db = database) {
      ...
      const result = await db.query(`INSERT INTO messages(message_id, conversation_id, global_user_id, project_scope, group_scope, thread_scope, direction, content, provenance)
        ...
        WHERE c.conversation_id=$2 AND c.global_user_id=$3 AND c.project_scope=$4
          AND c.group_scope IS NOT DISTINCT FROM $5 AND c.thread_scope IS NOT DISTINCT FROM $6 RETURNING *`, [messageId, conversationId, globalUserId, projectScope, groupScope, threadScope, direction, JSON.stringify(content), JSON.stringify(provenance)]);
      if (result.rowCount === 0) throw new Error('conversation scope mismatch');
      return result.rows[0];
    },
  ...
  const memory = Object.freeze({
    async put({ memoryId, scope, layer, key, value, provenance, trust = 'unverified', confirmed = false, tags = [], confidence = null, expiresAt = null }, db = database) {
      const [globalUserId, projectScope, groupScope, threadScope] = scopeValues(scope);
      await users.upsert({ globalUserId }, db);
      const result = await db.query(`INSERT INTO memory_records(memory_id,global_user_id,project_scope,group_scope,thread_scope,memory_layer,memory_key,value,provenance,trust,confirmed,tags,confidence,expires_at)
        VALUES ($1,$2,$3,$4,$5,$6,$7,$8::jsonb,$9::jsonb,$10,$11,$12::jsonb,$13,$14)
        ON CONFLICT(memory_id) DO UPDATE SET value=EXCLUDED.value,provenance=EXCLUDED.provenance,trust=EXCLUDED.trust,confirmed=EXCLUDED.confirmed,tags=EXCLUDED.tags,confidence=EXCLUDED.confidence,expires_at=EXCLUDED.expires_at,updated_at=now()
        WHERE memory_records.global_user_id=EXCLUDED.global_user_id AND memory_records.project_scope=EXCLUDED.project_scope
          AND memory_records.group_scope IS NOT DISTINCT FROM EXCLUDED.group_scope AND memory_records.thread_scope IS NOT DISTINCT FROM EXCLUDED.thread_scope RETURNING *`, [memoryId, globalUserId, projectScope, groupScope, threadScope, layer, key, JSON.stringify(value), JSON.stringify(provenance), trust, confirmed, JSON.stringify(tags), confidence, expiresAt]);
      if (result.rowCount === 0) throw new Error('memory scope mismatch');
      return result.rows[0];
    },
    async list({ scope, layers, keys = [], includeExpired = false }, db = database) {
      const [globalUserId, projectScope, groupScope, threadScope] = scopeValues(scope);
      const result = await db.query(`SELECT * FROM memory_records WHERE global_user_id=$1 AND project_scope=$2
        ...
        AND memory_layer = ANY($5::text[]) AND ($6::text[] = '{}'::text[] OR memory_key = ANY($6::text[]))
        AND ($7::boolean OR expires_at IS NULL OR expires_at > now()) ORDER BY memory_layer,memory_key,created_at,memory_id`, [globalUserId, projectScope, groupScope, threadScope, layers, keys, includeExpired]);
      return result.rows;
    }
  });
  ...
  const automation = Object.freeze({
    async putTask({ taskId, scope, status, payload, approvalState = {} }, db = database) {
      ...
      const result = await db.query(`INSERT INTO tasks(task_id,global_user_id,project_scope,group_scope,thread_scope,status,payload,approval_state)
        ...
        WHERE tasks.global_user_id=EXCLUDED.global_user_id AND tasks.project_scope=EXCLUDED.project_scope
          AND tasks.group_scope IS NOT DISTINCT FROM EXCLUDED.group_scope AND tasks.thread_scope IS NOT DISTINCT FROM EXCLUDED.thread_scope RETURNING *`, [taskId, globalUserId, projectScope, groupScope, threadScope, status, JSON.stringify(payload), JSON.stringify(approvalState)]);
    ...
    async putExecution({ executionId, taskId = null, status, attempt = 0, state = {}, error = null }, db = database) {
      const result = await db.query(`INSERT INTO execution_states(execution_id,task_id,status,attempt,state,error,started_at)
        ...
        ON CONFLICT(execution_id) DO UPDATE SET status=EXCLUDED.status,attempt=EXCLUDED.attempt,state=EXCLUDED.state,error=EXCLUDED.error,updated_at=now() RETURNING *`, [executionId, taskId, status, attempt, JSON.stringify(state), error === null ? null : JSON.stringify(error)]);
      return result.rows[0];
    }
  });
  ...
  const observability = Object.freeze({
    async record(event, db = database) {
      const result = await db.query(`INSERT INTO observability_events(channel,event_class,trace_id,request_id,global_user_id,project_scope,stage,outcome,payload)
        VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb) RETURNING *`, [event.channel, event.eventClass, event.traceId ?? null, event.requestId ?? null, event.globalUserId ?? null, event.projectScope ?? null, event.stage ?? null, event.outcome ?? null, JSON.stringify(redact(event.payload ?? {}))]);
      return result.rows[0];
    }
  });
    ...
    async put({ domainId, recordId, scope, payload }, db = database) {
      ...
      const result = await db.query(`INSERT INTO domain_records(domain_id,record_id,global_user_id,project_scope,group_scope,thread_scope,payload)
        ...
        WHERE domain_records.global_user_id=EXCLUDED.global_user_id AND domain_records.project_scope=EXCLUDED.project_scope
          AND domain_records.group_scope IS NOT DISTINCT FROM EXCLUDED.group_scope AND domain_records.thread_scope IS NOT DISTINCT FROM EXCLUDED.thread_scope RETURNING *`, [domainId, recordId, globalUserId, projectScope, groupScope, threadScope, JSON.stringify(payload)]);
