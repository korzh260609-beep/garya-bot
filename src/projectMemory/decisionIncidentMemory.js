...
function decisionView(fact) {
  return deepFreeze({
    ...
    decision: fact.fact.decision,
    rationale: fact.fact.rationale,
    alternatives: clone(fact.fact.alternatives ?? []),
    consequences: clone(fact.fact.consequences ?? []),
    validFrom: fact.validFrom,
    validTo: fact.validTo ?? null,
    current: !fact.validTo && !fact.successorMemoryId && fact.lifecycleState === 'active',
    provenance: clone(fact.source),
    trust: fact.trust,
...
function incidentView(fact, retrieval = null) {
  return deepFreeze({
    ...
    occurredAt: fact.fact.occurredAt ?? fact.validFrom,
    resolvedAt: fact.fact.resolvedAt ?? null,
    provenance: clone(fact.source),
    trust: fact.trust,
    confirmed: fact.confirmed,
    retrieval: retrieval ? clone(retrieval) : null,
    advisoryOnly: true,
    provesLiveRootCause: false
  });
}
...
export function createProjectDecision(input = {}) {
  ...
  const fact = {
    ...
    alternatives: strings(input.alternatives, 'alternatives'),
    consequences: strings(input.consequences, 'consequences')
  };
  return baseFact(input, {
    namespace: createProjectMemoryNamespace(projectKey, 'decisions'),
...
export function createProjectIncident(input = {}) {
  ...
  if (rootCause && !rootCauseEvidenceConfirmed) {
    throw error('project-memory-incident-root-cause-unconfirmed', 'incident rootCause requires evidence confirmation');
  }
  if (!rootCause && rootCauseEvidenceConfirmed) {
    throw error('project-memory-incident-root-cause-missing', 'rootCauseEvidenceConfirmed requires rootCause');
  }
  ...
  const fact = {
    status,
    symptom: required(input.symptom, 'symptom'),
    rootCause,
    rootCauseEvidenceConfirmed,
    fix,
    affectedComponents: strings(input.affectedComponents, 'affectedComponents'),
    prevention: strings(input.prevention, 'prevention'),
    occurredAt: occurredAt ? new Date(occurredAt).toISOString() : null,
    resolvedAt: resolvedAt ? new Date(resolvedAt).toISOString() : null
  };
  return baseFact(input, {
    namespace: createProjectMemoryNamespace(projectKey, 'incidents'),
    factType: 'incident',
    fact,
    pm310Kind: 'incident'
  });
...
export function createProjectDecisionIncidentMemory({ retrieval, contextGuard, temporalHistory, authorize } = {}) {
  if (!retrieval?.search) throw new TypeError('retrieval.search is required');
  if (!contextGuard?.build) throw new TypeError('contextGuard.build is required');
  ...
  async function getCurrentDecision({ actor, projectKey, entityKey } = {}) {
    ...
    const records = await temporalHistory.getCurrent({
      projectKey: project,
      namespace: createProjectMemoryNamespace(project, 'decisions'),
      factType: 'architecture-decision',
      entityKey: entity,
      limit: 10
    });
  ...
  async function getDecisionAt({ actor, projectKey, entityKey, at } = {}) {
    ...
    const records = await temporalHistory.getAt({
      projectKey: project,
      namespace: createProjectMemoryNamespace(project, 'decisions'),
      factType: 'architecture-decision',
      entityKey: entity,
  ...
  async function explainDecision({ actor, projectKey, entityKey } = {}) {
    ...
    return deepFreeze({
      kind: 'ProjectDecisionExplanation',
      contractVersion: PROJECT_MEMORY3_DECISION_INCIDENT_CONTRACT_VERSION,
      ...decision,
  ...
  async function getDecisionHistory({ actor, projectKey, memoryId } = {}) {
    ...
    return deepFreeze(chain.filter((record) => record.factType === 'architecture-decision').map(decisionView));
  }
  ...
  async function findIncidentGuidance({ actor, projectKey, query, limit = 5, queryEmbedding = null, modelKey = null } = {}) {
    ...
    const retrievalResult = await retrieval.search({
      ...
      queryEmbedding,
      modelKey,
      namespaces: [createProjectMemoryNamespace(project, 'incidents')],
      factTypes: ['incident'],
    ...
    const guarded = await contextGuard.build({
      ...
      projectKey: project,
      retrievalResult,
      allowedNamespaces: [createProjectMemoryNamespace(project, 'incidents')],
      allowedTrust: ['verified', 'confirmed'],
      allowedLifecycleStates: ['active', 'archived'],
      includeProposed: false,
      maxFacts: Math.max(1, Math.min(Number(limit) || 5, 12)),
      maxTokens: 2000
    });
    const byId = new Map(retrievalResult.results.map((item) => [item.record.memoryId, item]));
    const guidance = guarded.facts.map((fact) => {
      const original = byId.get(fact.memoryId)?.record;
      return incidentView(original, fact.retrieval);
    }).filter(Boolean);
    return deepFreeze({
      kind: 'HistoricalIncidentGuidance',
      contractVersion: PROJECT_MEMORY3_DECISION_INCIDENT_CONTRACT_VERSION,
      projectKey: project,
      query: text,
      advisoryOnly: true,
      ...
      incidents: guidance,
      conflictSummary: clone(guarded.conflictSummary)
    });
  }
