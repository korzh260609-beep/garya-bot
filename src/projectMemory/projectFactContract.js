...
export const PROJECT_MEMORY3_CONFIRMATION_STATES = Object.freeze([
  'proposed',
  'confirmed',
  'rejected'
]);
...
const RESERVED_AUTHORITY_KEY_SET = new Set(
  PROJECT_MEMORY3_RESERVED_AUTHORITY_KEYS.map(normalizeKey)
);
...
function normalizeProjectKey(value) {
  ...
  if (!/^[a-z0-9][a-z0-9._-]{0,63}$/.test(projectKey)) {
    throw new TypeError('projectKey contains unsupported characters');
  }
  return projectKey;
}
...
function assertNoForbiddenStructuredKeys(value, path = 'fact') {
  ...
  if (Array.isArray(value)) {
    value.forEach((item, index) => assertNoForbiddenStructuredKeys(item, `${path}[${index}]`));
    return;
  }
...
function uniqueStrings(values, name) {
  ...
  return Object.freeze([...new Set(values.map((value) => requiredString(value, name)))]);
}
...
export function createProjectMemoryNamespace(projectKeyInput, domainInput) {
  ...
  if (!PROJECT_MEMORY3_DOMAINS.includes(domain)) {
    throw new TypeError(`unsupported Project Memory domain: ${domain}`);
  }
...
export function createProjectMemoryNamespaces(projectKeyInput) {
  ...
  return Object.freeze(Object.fromEntries(
    PROJECT_MEMORY3_DOMAINS.map((domain) => [domain, createProjectMemoryNamespace(projectKey, domain)])
  ));
}
...
function validateFactType(value) {
  ...
  if (!PROJECT_MEMORY3_FACT_TYPES.includes(factType)) {
    throw new TypeError(`unsupported project fact type: ${factType}`);
  }
  return factType;
}
...
function validateLifecycle(value) {
  ...
  if (!MEMORY2_LIFECYCLE_STATES.includes(lifecycleState)) {
    throw new TypeError(`unsupported lifecycle state: ${lifecycleState}`);
  }
...
function validateConfirmationState(value, confirmed) {
  ...
  if (!PROJECT_MEMORY3_CONFIRMATION_STATES.includes(state)) {
    throw new TypeError(`unsupported confirmation state: ${state}`);
  }
  if (confirmed !== (state === 'confirmed')) {
    throw new TypeError('confirmed flag must match confirmationState');
  }
  return state;
}
...
export function createProjectFact(input = {}, { clock = () => new Date() } = {}) {
  ...
  if (!traceId && !sourceEventId) {
    throw new TypeError('traceId or sourceEventId is required');
  }
  ...
  if (validTo && Date.parse(validTo) <= Date.parse(validFrom)) {
    throw new TypeError('validTo must be later than validFrom');
  }
  ...
  return deepFreeze({
    ...
    createdAt,
    updatedAt,
    supersededAt: isoTimestamp(input.supersededAt, 'supersededAt'),
    successorMemoryId: optionalString(input.successorMemoryId),
    relationKeys,
    tags,
    recordVersion,
    semanticFingerprint: fingerprint(canonicalPayload),
    metadata
  });
}
...
export function selectProjectFactsForProject(records, { projectKey, namespaces = null } = {}) {
  ...
  const allowedNamespaces = namespaces == null
    ? null
    : new Set(namespaces.map((namespace) => assertProjectMemoryNamespaceForProject(namespace, normalizedProjectKey).namespace));
