...
export function createLocalProductionHarness({ env = {}, interpretationResolver, fetchImpl = globalThis.fetch, clock = () => new Date() } = {}) {
  ...
  const domainDispatcher = async ({ domainId, capability, input, request }) => controlPlane.domainRuntime.execute({
    domainId,
    capability,
    input,
    identityContext: request.actor,
    scopeContext: request.scope,
  ...
  const capabilityExecutor = createFeatureFlaggedCapabilityExecutor({ executor: baseCapabilityExecutor, featureFlags });
  ...
  const runtime = createProductionRuntime({ config, semanticPipeline, actionGate, capabilityRegistry, capabilityExecutor, domainRuntime: controlPlane.domainRuntime, contractVersioning: controlPlane.contractVersioning, observability, languageContextService, conversationContextService, userSettingsService, policyLayer, resourceAuthorityRegistry, resources });
