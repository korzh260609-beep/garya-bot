...
import { createAICostIntelligence } from '../ai/costIntelligence.js';
import { createPostgresCostIntelligenceStore } from '../ai/postgresCostIntelligenceStore.js';
import { createOpenAIAdminUsageClient } from '../ai/openAIAdminUsageClient.js';
import { createOpenAICostReconciler, createCostReconciliationResource } from '../ai/costReconciliation.js';
...
import { createGitHubAccess, GITHUB_ACTION } from '../integrations/github/githubAccess.js';
...
export function createLocalProductionHarness({ env = {}, interpretationResolver, fetchImpl = globalThis.fetch, clock = () => new Date() } = {}) {
  ...
  const costStore = persistence ? createPostgresCostIntelligenceStore({ database: persistence.database }) : null;
  const costIntelligence = createAICostIntelligence({ persistence: costStore, now: clock });
  const adminCostEnabled = Boolean(persistence && env.OPENAI_ADMIN_API_KEY);
  const openAIAdminClient = adminCostEnabled ? createOpenAIAdminUsageClient({ credentialManager, credentialAccessContext, fetchImpl, baseUrl: env.OPENAI_ADMIN_BASE_URL ?? 'https://api.openai.com/v1' }) : null;
  const costReconciler = openAIAdminClient ? createOpenAICostReconciler({ client: openAIAdminClient, store: costStore, clock }) : null;
  const costReconciliation = createCostReconciliationResource({ reconciler: costReconciler ?? { async reconcile() { return null; } }, enabled: adminCostEnabled, intervalMs: Number(env.OPENAI_COST_RECONCILIATION_INTERVAL_MS) > 0 ? Number(env.OPENAI_COST_RECONCILIATION_INTERVAL_MS) : 86400000, costIntelligence });
  ...
  const productionAI = !interpretationResolver && aiRequested(env) ? createProductionAI({ env, fetchImpl, configurationPolicy: basePolicy, credentialManager, credentialAccessContext, connectionRegistry, connectionAccessContext, costIntelligence }) : null;
  const githubAccess = createGitHubAccess({ env, aiRouter: productionAI?.aiRouter ?? null, ownerGlobalUserId: ownerSecurityConfig.monarchGlobalUserId, fetchImpl, clock });
  ...
  const authorizedActionNames = Object.freeze([...capabilityNames, GITHUB_ACTION]);

  const selfKnowledgeSources = createDeploymentSelfKnowledgeSources({ config, capabilityNames, githubAccess, persistence, productionAI, connectionRegistry, resourceAuthorityRegistry, conversationContextService, userSettingsService, languageContextService, temporalService, featureFlags, eventBus: controlPlane.eventBus, contractVersioning: controlPlane.contractVersioning, domainRuntime: controlPlane.domainRuntime });
