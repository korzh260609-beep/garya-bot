...
export function createProductionTelegramIdentityResolver({ persistence, projectScope, monarchTelegramUserId = null } = {}) {
  ...
  return async ({ platformFacts, scopeFacts }) => {
    ...
    let link = await persistence.repositories.identities.resolve(platform, platformUserId);
    const globalUserId = link?.global_user_id ?? `telegram:${platformUserId}`;
    if (!link) {
      link = await persistence.repositories.identities.link({ platform, platformUserId, globalUserId, metadata: { source: 'telegram-production' } });
    }
...
export async function createRenderWebApplication({ env = process.env, fetchImpl = globalThis.fetch } = {}) {
  const harness = createLocalProductionHarness({ env, fetchImpl });
  ...
  const identityResolver = createProductionTelegramIdentityResolver({
    ...
    monarchTelegramUserId: env.SG_MONARCH_TELEGRAM_USER_ID
  });
