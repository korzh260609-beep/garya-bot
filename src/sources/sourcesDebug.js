...
// Заглушка для просмотра списка источников (Этап 5)
...
export async function getAllSourcesSafe() {
  try {
    const result = await pool.query(
      `
        SELECT key, name, type, is_enabled as enabled, url, config
        FROM sources
        ORDER BY key
      `
    );
...
export async function ensureDefaultSources() {
  console.warn(
    "⚠️ ensureDefaultSources() вызвано из sourcesDebug.js — это заглушка. Используйте sources.js"
  );
  return [];
}
...
export async function runSourceDiagnosticsOnce() {
  console.warn(
    "⚠️ runSourceDiagnosticsOnce() вызвано из sourcesDebug.js — это заглушка. Используйте sources.js"
  );
...
export async function fetchFromSourceKey() {
  console.warn(
    "⚠️ fetchFromSourceKey() вызвано из sourcesDebug.js — это заглушка. Используйте sources.js"
  );
  return null;
}
