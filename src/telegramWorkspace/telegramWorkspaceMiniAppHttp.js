...
async function save(ns){try{say('Проверяю изменение…');const next=nextConfig(ns);const p=await call('/propose',{workspaceId:workspaceEl.value,namespace:ns,nextConfig:next});const paths=(p.changedPaths||[]).join(', ')||'без изменений';if(!confirm('Подтвердить изменение?\\nРиск: '+p.risk+'\\nИзменения: '+paths)) {say('Изменение отменено.');return;}await call('/apply',{workspaceId:workspaceEl.value,namespace:ns,nextConfig:next,requestId:p.requestId,confirmed:true});say('Сохранено.');await loadWorkspace();}catch(e){say('Ошибка: '+e.message,true)}}
async function loadWorkspace(){const detail=await call('/workspace',{workspaceId:workspaceEl.value});render(detail)}
workspaceEl.onchange=()=>loadWorkspace().catch(e=>say('Ошибка: '+e.message,true));
document.getElementById('history').onclick=async()=>{try{const m=configMap(current?.configs);const names=Object.keys(m);if(!names.length){historyOut.textContent='История пока пуста.';return;}const blocks=[];for(const ns of names){const rows=await call('/history',{workspaceId:workspaceEl.value,namespace:ns,limit:5});blocks.push(ns+': '+((rows||[]).map(x=>'v'+x.version).join(', ')||'пусто'));}historyOut.textContent=blocks.join('\\n')}catch(e){say('Ошибка: '+e.message,true)}};
(async()=>{try{if(!initData)throw new Error('Открой Mini App из Telegram.');const b=await call('/bootstrap');workspaceRows=b.workspaces||[];if(!workspaceRows.length){say('Нет доступных групп или каналов.');return;}workspaceEl.innerHTML=workspaceRows.map(w=>'<option value="'+w.workspaceId.replaceAll('"','&quot;')+'">'+workspaceLabel(w).replaceAll('<','&lt;')+'</option>').join('');appEl.classList.remove('hidden');say('Подключено.');await loadWorkspace()}catch(e){say('Ошибка запуска: '+e.message,true)}})();
</script>
</body></html>`;
}
