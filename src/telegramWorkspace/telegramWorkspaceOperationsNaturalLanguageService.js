...
export function createTelegramWorkspaceOperationsNaturalLanguageService({
  ...
  async function recentConversation(update, actor) {
    const service = operationsService.conversationContextService;
    if (typeof service?.resolveTurn !== 'function') return [];
    const message = messageFrom(update);
    try {
      const turn = await service.resolveTurn({
        globalUserId: actor.actorGlobalUserId,
        projectScope: project,
        groupScope: ['group','supergroup'].includes(actor.chatType) ? actor.chatId : null,
        threadScope: message?.message_thread_id == null ? null : String(message.message_thread_id),
        transport: 'telegram',
        transportSessionId: null,
        platformMessageId: message?.message_id == null ? null : String(message.message_id),
        replyToMessageId: message?.reply_to_message?.message_id == null ? null : String(message.reply_to_message.message_id),
        text: semanticText(update),
        metadata: { source: 'telegram-workspace-operation' }
      });
      return Array.isArray(turn?.recentTurns) ? turn.recentTurns : [];
    } catch {
      return [];
    }
  }
  ...
  async function interpret(update, actor, candidates, forced, refs) {
    ...
    const conversationTurns = await recentConversation(update, actor);
