...
describe("SG 2.2 Render entrypoint", () => {
  ...
  it("isolates public DMs and bounds paid model context on every boot", async () => {
    ...
    expect(script).toContain(
      "for plugin_file in index.ts register.ts personal-memory-tools.ts resource-memory-tools.ts project-memory-tools.ts scoped-memory-entries.ts cost-diagnostics.ts render-tools.ts openclaw.plugin.json package.json",
    );
  });
});
