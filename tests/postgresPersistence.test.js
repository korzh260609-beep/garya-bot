...
integration('Block 12 upgrades an SG 2.0 database in place without deleting legacy data', async () => {
  ...
  await database.query(`
    DROP TABLE IF EXISTS telegram_updates, dead_letter_tasks, domain_records, observability_events,
  ...
  assert.equal(migrated.applied.length, 5);
  assert.equal(migrated.total, 5);
...
integration('Block 12 PostgreSQL persistence is durable, isolated and atomic', async () => {
  ...
  assert.equal(migrationRepeat.total, 5);
